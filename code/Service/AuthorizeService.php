...
class AuthorizeService extends PaymentService
{
    ...
    public function initiate($data = array()) {
        if ($this->payment->Status !== 'Created') {
            throw new InvalidStateException('Cannot initiate authorization with this payment. Status is not "Created"');
        }
        ...
        if ($serviceResponse->isRedirect() || $serviceResponse->isAwaitingNotification()) {
            ...
            $this->createMessage(
                $serviceResponse->isRedirect() ? 'AuthorizeRedirectResponse' : 'AuthorizedResponse',
                $response
            );
    ...
    public function complete($data = array(), $isNotification = false)
    {
        ...
        if(!$this->payment->Status === 'PendingAuthorization'){
            throw new InvalidStateException('Cannot authorize this payment. Status is not "PendingAuthorization"');
        }
        ...
        // authorize and completeAuthorize should use the same data
        $gatewayData = $this->gatherGatewayData($data);
